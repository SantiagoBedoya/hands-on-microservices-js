...
const getAccounts = async (req, res) => {
  ...
  res
    .status(200)
    .json({ success: true, accounts: result.map((x) => mapToResponse(x)) });
};
...
const createAccount = async (req, res) => {
  ...
  res.status(201).json({ success: true, account: mapToResponse(result) });
};
  ...
  if (result.error) {
    switch (result.code) {
      case accountService.errorCodes.NO_VALID_DATA_TO_UPDATE:
        res.status(400).jso({ success: false, message: result.error });
        return;
